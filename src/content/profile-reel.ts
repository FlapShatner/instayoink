...
async function fetchVideoURL(
  ...
  const posterUrl = (
    timeNodes[timeNodes.length - 1].parentNode!.parentNode as any
  ).href;
  const posterPattern = /\/([^/?]*)\?/;
...
async function getUrl() {
  ...
  if (mediaList.length === 0) {
    ...
    url = res?.url;
    if (!url) {
      const videoElem: HTMLVideoElement | null = containerNode.querySelector(
        'article  div > video'
      );
      const imgElem = containerNode.querySelector(
...
export async function handleProfileReel(target: HTMLAnchorElement) {
  ...
  async function getDataFromLocal() {
    const { profile_reels_edges_data, id_to_username_map } =
    ...
    if (media) {
      ...
      const times =
        target.parentElement?.parentElement?.parentElement?.querySelectorAll(
          'time'
        );
      const time = times
        ? times[times.length - 1]?.getAttribute('datetime')
        : undefined;
      final({
        url: url,
        username:
