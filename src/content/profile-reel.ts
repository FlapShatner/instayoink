...
export async function handleProfileReel(target: HTMLAnchorElement) {
 ...
 async function getDataFromLocal() {
  const { profile_reels_edges_data, id_to_username_map } = await chrome.storage.local.get(['profile_reels_edges_data', 'id_to_username_map'])
  ...
  if (media) {
   ...
   final({
    url: url,
    username: (new Map(id_to_username_map).get(media.user.id) as string) || document.querySelector('a')?.getAttribute('href')?.replace(/\//g, ''),
