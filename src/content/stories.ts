...
function findStories(
  obj: Record<string, any>
): Stories.XdtApiV1FeedReelsMedia | undefined {
  for (const key in obj) {
...
export async function storyOnClicked(target: HTMLAnchorElement) {
  ...
  const { setting_format_use_indexing } = await chrome.storage.sync.get([
  ...
  const handleMedia = (item: Stories.ReelsMedum, mediaIndex: number) => {
    ...
    if (target.className.includes('download-btn')) {
      downloadResource({
        ...
        fileId: setting_format_use_indexing
          ? `${item.id}_${mediaIndex + 1}`
          : getMediaName(url),
      });
    } else {
      openInNewTab(url);
  ...
  const { stories_reels_media } = await chrome.storage.local.get([
    'stories_reels_media',
  ]);
    ...
    for (const script of window.document.scripts) {
      try {
        ...
        if (innerHTML.includes('rootView')) {
          ...
          const id =
            rootViewData?.props.media_owner_id || rootViewData?.props.id;
