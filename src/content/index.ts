...
setInterval(() => {
  ...
  const isPostDetailWithNameInUrl =
    pathnameList.length === 3 && pathnameList[1] === 'p'; // https://www.instagram.com/frankinjection/p/CwAb4TEoRE_/?img_index=1
  const isReelDetailWithNameInUrl =
    pathnameList.length === 3 && pathnameList[1] === 'reel'; // https://www.instagram.com/philsnelgrove/reel/B5GeRJoBAc1/
  ...
  if (pathname === '/' || isFeedPage) {
    ...
    for (let i = 0; i < articleList.length; i++) {
      ...
      if (tagNode) {
        articleList[i]
          .querySelectorAll<HTMLImageElement>('ul li img')
          .forEach((img) => {
            const emptyNode = img.parentElement?.nextElementSibling;
            if (
              emptyNode instanceof HTMLDivElement &&
        ...
        articleList[i]
          .querySelectorAll<HTMLImageElement>(':scope img')
          .forEach((img) => {
      ...
      if (
        ...
        addCustomBtn(
          likeBtn.parentNode?.parentNode?.parentNode?.parentNode?.parentNode
  ...
  if (
    ...
    // handleVideo();
    const dialogNode =
      document.querySelector<HTMLDivElement>('div[role="dialog"]');
  ...
  if (pathname.startsWith('/stories/')) {
    ...
    const blockDiv = [
      ...document.querySelectorAll(
        'body>div:not(#splash-screen)>div>div>div>div'
      ),
    ].find((el) => {
    ...
    if (
      storyMenuBtn &&
      blockDiv?.getElementsByClassName(CLASS_CUSTOM_BUTTON).length === 0
    ) {
      addCustomBtn(
    ...
    chrome.storage.sync
      .get(['setting_enable_video_controls'])
      .then((result) => {
  ...
  if (pathname.startsWith('/reel/')) {
    ...
    if (
      commentBtn &&
      node.getElementsByClassName(CLASS_CUSTOM_BUTTON).length === 0
    ) {
      addCustomBtn(
  ...
  if (
    profileHeader &&
    profileHeader.getElementsByClassName(CLASS_CUSTOM_BUTTON).length === 0
  ) {
  ...
  if (
    ...
    const postsRows = document
      ...
      ?.nextElementSibling?.querySelectorAll(
        `:scope>div>div>div ${pathnameList.length === 1 ? '' : '>div'}`
      );
