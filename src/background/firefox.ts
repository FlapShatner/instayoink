  ...
  switch (details.url) {
    ...
    default:
      if (
        ...
        const { reels, reels_media } = await browser.storage.local.get([
          'reels',
          'reels_media',
        ]);
        const newArr = (reels_media || []).filter(
          (i: ReelsMedia.ReelsMedum) =>
            !(jsonData as ReelsMedia.Root).reels_media.find(
              (j) => j.id === i.id
            )
        );
...
async function listenThreads(
  ...
  if (details.url === 'https://www.threads.net/graphql/query') {
    ...
    if (Array.isArray(jsonData.data?.mediaData?.edges)) {
      const data = jsonData.data.mediaData.edges
        .map((i: any) => i.node.thread_items)
        .flat();
      await addThreads(data);
    }
    if (Array.isArray(jsonData.data?.data?.edges)) {
      const data = jsonData.data.data.edges
        .map((i: any) => i.node.thread_items)
        .flat();
      await addThreads(data);
    }
    ...
    if (Array.isArray(jsonData.data?.searchResults?.edges)) {
      const data = jsonData.data.searchResults.edges
        .map((i: any) => i.node.thread.thread_items)
        .flat();
      await addThreads(data);
    }
  }
  ...
  if (details.url === 'https://www.threads.net/ajax/route-definition/') {
    ...
    if (result && Array.isArray(result.edges)) {
      await addThreads(
        result.edges.map((i: any) => i.node.thread.thread_items).flat()
      );
    }
  }
}
    ...
    try {
      ...
      try {
        ...
        if (
          ...
          const { stories_user_ids, id_to_username_map } =
        ...
        if (
          details.url === 'https://www.threads.net/ajax/route-definition/' &&
...
browser.webRequest.onBeforeRequest.addListener(
  (details) => {
    try {
      ...
      if (
        method === 'POST' &&
        url === 'https://www.threads.net/ajax/route-definition/'
      ) {
        listener(details);
      }
    } catch {}
